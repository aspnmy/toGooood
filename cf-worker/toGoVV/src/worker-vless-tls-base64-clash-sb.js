...
let userID = 'd619850b-8e14-48d1-85c4-6c2b4a10fd3c';
// 生成配置文件的 CF 优选 IP
const bestCFIP = 'www.gov.se';

// 用于 CF 网站的代理 IP
//const proxyIPs = ['nd1.ddt.shdrr.org'];
//let proxyIP = proxyIPs[Math.floor(Math.random() * proxyIPs.length)];
let proxyIP = "nd4.ddt.shdrr.org";
let dohURL = 'https://security.cloudflare-dns.com/dns-query';
            ...
            new WritableStream({
                async write(chunk, controller) {
                    ...
                    const {
                        ...
                        isUDP,
                    } = await processVlessHeader(chunk, env.UID);
...
async function processVlessHeader(vlessBuffer, userID) {
    ...
    const uuids = env.UID.includes(',') ? env.UID.split(',') : [env.UID];
            ...
            new WritableStream({
                async write(chunk) {
                    const resp = await fetch(
                        env.DNS_RESOLVER_URL, // dns server url
                        {
                            method: 'POST',
                            headers: {
...
function getVLESSConfig(userID, hostName) {
    ...
    return `
下面是不支持Tls部署形式,如果希望尝试可用下面这个配置连接:
